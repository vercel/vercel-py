...
from vercel.projects.projects import get_projects_async, create_project_async, update_project_async, delete_project_async
...
class TestProjectsRealAPI:
    ...
    def test_get_projects_real_api(self, team_id):
        ...
        assert isinstance(project["accountId"], str), f"Expected string, got {type(project['accountId'])}"
        assert isinstance(project["createdAt"], int), f"Expected int, got {type(project['createdAt'])}"
        assert isinstance(project["updatedAt"], int), f"Expected int, got {type(project['updatedAt'])}"
        
        # Validate ID formats
        assert project["id"].startswith("prj_"), f"Project ID should start with 'prj_', got: {project['id']}"
        assert project["accountId"].startswith("team_"), f"Account ID should start with 'team_', got: {project['accountId']}"
        
        # Validate timestamps are reasonable (after 2020)
        assert project["createdAt"] > 1577836800000, f"Created timestamp too old: {project['createdAt']}"
        assert project["updatedAt"] > 1577836800000, f"Updated timestamp too old: {project['updatedAt']}"
        ...
        assert isinstance(pagination["count"], int), f"Expected int, got {type(pagination['count'])}"
    ...
    def test_create_project_real_api(self, team_id, test_project_name):
        ...
        assert isinstance(result["accountId"], str), f"Expected string, got {type(result['accountId'])}"
        assert isinstance(result["createdAt"], int), f"Expected int, got {type(result['createdAt'])}"
        assert isinstance(result["updatedAt"], int), f"Expected int, got {type(result['updatedAt'])}"
        
        # Validate values match what we sent
        assert result["name"] == test_project_name, f"Expected {test_project_name}, got {result['name']}"
        assert result["accountId"] == team_id, f"Expected {team_id}, got {result['accountId']}"
        
        # Validate ID format
        assert result["id"].startswith("prj_"), f"Project ID should start with 'prj_', got: {result['id']}"
        
        # Validate timestamps are recent (within last minute)
        current_time = int(time.time() * 1000)
        assert result["createdAt"] > current_time - 60000, f"Created timestamp too old: {result['createdAt']}"
        assert result["updatedAt"] > current_time - 60000, f"Updated timestamp too old: {result['updatedAt']}"
    ...
    def test_update_project_real_api(self, team_id, test_project_name):
        ...
        try:
            ...
            assert result["updatedAt"] >= created_project["createdAt"], "UpdatedAt should be >= createdAt"
    ...
    async def test_create_project_async_real_api(self, team_id, test_project_name):
        ...
        project_body = {
            "name": f"{test_project_name}-async",
            "framework": "nextjs"
        }
        ...
        # Validate values
        assert result["name"] == f"{test_project_name}-async", f"Expected {test_project_name}-async, got {result['name']}"
        assert result["accountId"] == team_id, f"Expected {team_id}, got {result['accountId']}"
        
        # Validate ID format
        assert result["id"].startswith("prj_"), f"Project ID should start with 'prj_', got: {result['id']}"
        
        print(f"✅ Real async API test passed: Created project {result['name']} with ID {result['id']}")
    ...
    def test_error_handling_real_api(self, team_id):
        ...
        assert "Failed to delete project" in error_message, f"Expected 'Failed to delete project' in: {error_message}"
    ...
    def test_full_crud_workflow_real_api(self, team_id, test_project_name):
        ...
        try:
            ...
            assert test_project_name in project_names, f"Project {test_project_name} not found in list"
            print(f"✅ Verified project exists in list")
            ...
            print(f"✅ Updated project framework")
            ...
            assert test_project_name not in project_names_after, f"Project {test_project_name} still exists after deletion"
            print(f"✅ Verified project was deleted")
            
            print(f"🎉 Full CRUD workflow test passed!")
