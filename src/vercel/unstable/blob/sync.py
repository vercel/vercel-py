"""Synchronous mirror of the experimental Vercel Blob API."""
...
from vercel._internal.iter_coroutine import iter_coroutine as _iter_coroutine
...
from vercel._internal.unstable.blob.runtime_common import (
    SyncBlobEntry as BlobEntry,
    SyncBlobObjectEntry as BlobObjectEntry,
    SyncBlobPrefixEntry as BlobPrefixEntry,
)
...
def _service() -> _service_impl.BlobService:
    return _service_impl.get_blob_service(_session_impl.get_active_sync_session())
...
def open(
    ...
    *,
    ...
    Reader streams pin the object ETag observed at open time. Writer streams
    stage mutations locally and publish the replacement Blob when the stream
    closes successfully.
    ...
    Returns:
        The opened synchronous Blob stream.
    """
...
def stat(pathname: StrPath) -> BlobStatResult:
    ...
    """
    return _iter_coroutine(_service().stat(pathname))
...
def scandir(
    ...
    *,
    ...
    service = _service()
    source = cast(
        AsyncGenerator[_models.BlobEntryState, None],
        service.scandir(prefix=prefix, mode=mode, page_size=page_size, cursor=cursor),
    )

    def entries() -> Iterator[BlobEntry]:
        try:
            while True:
                try:
                    entry: _models.BlobEntryState = _iter_coroutine(source.__anext__())
...
def remove(pathname: StrPath, *, missing_ok: bool = False) -> None:
    ...
    """
    _iter_coroutine(_service().remove(pathname, missing_ok=missing_ok))


def rmtree(pathname: StrPath, *, missing_ok: bool = False) -> None:
    ...
    """
    _iter_coroutine(_service().rmtree(pathname, missing_ok=missing_ok))
