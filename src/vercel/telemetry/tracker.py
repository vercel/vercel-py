...
def track(event: str, **attrs: Any) -> None:
    """Generic track function that routes to TelemetryClient methods.

    Maps ``event`` -> ``TelemetryClient.track_{event}`` and filters kwargs
    to the method's signature to avoid unexpected arguments.
    """
    try:
        client = _get_telemetry_client()
        if client is None:
            return
        method_name = f"track_{event}"
        func = getattr(client, method_name, None)
        if func is None:
            return
        sig = inspect.signature(func)
        allowed = {k: v for k, v in attrs.items() if k in sig.parameters}
        func(**allowed)
    except Exception:
        # Silently fail - don't impact user's operation
        pass
...
def emit_telemetry(
    ...
    - ``when``: emit "before" the call, "after" the call.
    """
