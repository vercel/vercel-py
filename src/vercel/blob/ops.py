...
async def head(url_or_pathname: str, *, token: str | None = None) -> HeadBlobResultType:
    ...
    uploaded_at = parse_datetime(resp['uploadedAt']) if isinstance(resp.get('uploadedAt'), str) else resp['uploadedAt']
...
async def list_blobs(
    *,
    ...
    for b in resp.get('blobs', []):
        uploaded_at = parse_datetime(b['uploadedAt']) if isinstance(b.get('uploadedAt'), str) else b['uploadedAt']
        blobs_list.append(
            ListBlobItem(
                url=b['url'],
