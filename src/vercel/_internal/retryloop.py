# Adapted from Gel's edb/common/retryloop.py.
...
_DEFAULT_BACKOFF = const_backoff(0.5)
...
class RetryIteration:
    ...
    async def __aexit__(
        ...
        if self._loop._ignore is not None or self._loop._ignore_regexp is not None:
            ...
            assert self._loop._wait_for is not None or self._loop._wait_for_regexp is not None

            if et is not None:
                if (self._loop._wait_for is None or isinstance(e, self._loop._wait_for)) and (
