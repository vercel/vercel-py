...
async def test_reads_a_log_written_at_a_newer_spec_version(tmp_path, monkeypatch) -> None:
    ...
    # the version its World declares -- 7 since the sealed log, on the local
    # adapter the e2e lane runs on as well as the Vercel one -- and the Python
    # app under test replays it. Nothing here decodes by version (the payload
    # prefix says the format), and the row inherits the version of the event.
    world = _world(tmp_path, monkeypatch)
    result = await world.events_create(
        None,
        w.RunCreatedEvent(
    ...
    assert result.run is not None
    run_id = result.run.run_id
    assert result.run.spec_version == 7
    assert json.loads((world.data_dir / "runs" / f"{run_id}.json").read_text())["specVersion"] == 7
    assert [event.spec_version for event in (await world.events_list(run_id)).data] == [7]

    # Our own writes into the run leave its version alone.
    await world.events_create(run_id, w.RunStartedEvent())
    assert (await world.runs_get(run_id)).spec_version == 7
