...
async def checkout(amount: int) -> str:
    # Two writes from the body, the second of which removes a key: an unset is
    # an explicit `null` on the wire, which the TS reader requires and would
    ...
    await finish()
    await runtime.set_attributes({"temporary": None})
    return result
