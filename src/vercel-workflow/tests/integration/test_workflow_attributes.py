...
from vercel.workflow import FatalError, set_attributes, sleep
...
@registry.workflow
async def sets_attributes_and_forgets() -> str:
    # Dropped awaitables. The call is what records the write, so each one still
    # lands: the first two at the suspension the sleep creates, the last when
    # the body finishes.
...
async def test_a_dropped_awaitable_still_writes(world) -> None:
    """Including the last one, which no later suspension flushes."""
    run_id, result = await _run(world, sets_attributes_and_forgets)

    assert result == "completed"
    run = await world.runs_get(run_id)
    assert run.attributes == {"phase": "done", "mode": "fire-and-forget"}
...
async def test_a_rejected_write_does_not_wedge_the_run(world) -> None:
    ...
    assert "exceed limit 64" in outcomes["overCap"]
