...
class LocalWorld(w.World):
    ...
    def _events_create_impl(self, run_id: str | None, data: w.Event) -> w.EventResult:
        ...
        if data.event_type == "run_created" and hasattr(data, "event_data"):
            ...
            # Claimed after validation: a rejected write has to leave the
            # correlation id free, or its retry comes back as "already exists"
            # and the body waits for an event nobody writes. A step's write is
            # not replayed and carries no correlation id, so it needs no claim.
