"""Redis job store defaults and atomic scheduler coordination."""
...
if ARGV[5] ~= "" and redis.call("HGET", KEYS[4], "state") == "running"
then
...
class RedisJobCoordinator:
    ...
    def __init__(
        ...
        self._test_mode = bool(getattr(store, "_vercel_apscheduler_test_store", False))
        self._test_versions: dict[str, int] = {}
    ...
    def add_job(self, job: Any) -> None:
        if self._test_mode:
            ...
            self._test_versions[job.id] = self._next_test_revision()
            self._test_rearm(job, include_declarations=True)
            return
    ...
    def update_job(self, job: Any) -> None:
        if self._test_mode:
            ...
            self._test_versions[job.id] = self._next_test_revision()
            self._test_rearm(job, include_declarations=False)
            return
        result = self._write(_UPDATE_JOB_SCRIPT, job)
    ...
    def remove_job(self, job_id: str) -> None:
        if self._test_mode:
            ...
            self._next_test_revision()
            self._test_versions.pop(job_id, None)
            return
        result = _text(
            self.redis.eval(
                _REMOVE_JOB_SCRIPT,
    ...
    def get_due_jobs_with_revisions(
        ...
        if self._test_mode:
            return [
                (job, self._test_versions.get(job.id, 0)) for job in self.store.get_due_jobs(now)
            ]
        raw = self.redis.eval(
            _GET_DUE_WITH_REVISIONS_SCRIPT,
            3,
    ...
    def get_all_jobs_with_revisions(self) -> list[tuple[Any, int]]:
        if self._test_mode:
            return [(job, self._test_versions.get(job.id, 0)) for job in self.store.get_all_jobs()]
        raw = self.redis.eval(
            _GET_ALL_WITH_REVISIONS_SCRIPT,
            2,
            self.store.jobs_key,
    ...
    def cas_update_job(self, job: Any, expected_revision: int) -> bool:
        if self._test_mode:
            if self._test_versions.get(job.id, 0) != expected_revision:
                return False
            self._original_update_job(job)
            self._test_versions[job.id] = self._next_test_revision()
            return True
    ...
    def cas_remove_job(self, job_id: str, expected_revision: int) -> bool:
        if self._test_mode:
            if job_id not in getattr(self.store, "jobs", {}):
                return False
            if self._test_versions.get(job_id, 0) != expected_revision:
                return False
            self._original_remove_job(job_id)
            self._next_test_revision()
            self._test_versions.pop(job_id, None)
    ...
    def _write(self, script: str, job: Any) -> Any:
        state, score = self._serialized_job(job)
        ...
        return self.redis.eval(
            ...
            *self.keys,
            ...
            state,
            score,
            "1" if self.adapter.is_runtime_mutation else "0",
            candidate,
            now.isoformat(),
        )
    ...
    def _next_test_revision(self) -> int:
        self._test_revision += 1
        return self._test_revision
    ...
    def _test_rearm(
        ...
        *,
        ...
        if not include_declarations and not self.adapter.is_runtime_mutation:
            return
        next_run_time = getattr(job, "next_run_time", None)
        if next_run_time is None:
            return
        rearm: Callable[[datetime, datetime], Any] | None = getattr(
            self.driver,
            "rearm",
            None,
        )
