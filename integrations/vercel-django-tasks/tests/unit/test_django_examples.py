...
def test_chunks_example_uses_pyproject_subscriber_contract() -> None:
    ...
    pyproject = tomllib.loads((EXAMPLE_ROOT / "pyproject.toml").read_text(encoding="utf-8"))
    assert "Django>=6" in pyproject["project"]["dependencies"]
    assert pyproject["tool"]["vercel"]["entrypoint"] == "chunks_project.wsgi:application"
    assert pyproject["tool"]["vercel"]["subscribers"] == [{"entrypoint": "chunks_project.wsgi"}]

    wsgi = (EXAMPLE_ROOT / "chunks_project" / "wsgi.py").read_text(encoding="utf-8")
    assert "install_vercel_django_task_integration" not in wsgi
